"use client"; // පාවිච්චි කරන icons අනුව මේක අවශ්‍ය වෙන්න පුළුවන්
            ...
            <ul>
              {headerData.map((item, index) => (
                <li key={index} className="pb-4">
                  <Link
                    href={item.href || "#"}
                    className="text-white hover:text-primary text-17 transition-all"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div className="lg:col-span-2 md:col-span-3 col-span-6">
            ...
            <ul>
              {footerlabels.map((item, index) => (
                <li key={index} className="pb-4">
                  <Link
                    href={item.href || "#"} // මෙතන herf තිබුණ එක href ලෙස වෙනස් කළා
                    className="text-white hover:text-primary text-17 transition-all"
                  >
                    {item.label}
                  </Link>
          ...
          <div className="lg:col-span-4 md:col-span-4 col-span-6">
            ...
            <div className="relative lg:w-80%">
              <input
                ...
                className="bg-transparent border border-dark_border border-opacity-60 py-4 text-white rounded-lg w-full mt-6 px-6 outline-none focus:border-primary"
              />
              <Icon
                icon="tabler:send"
                width="24"
