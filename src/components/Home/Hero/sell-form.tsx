...
// Type error එක නැති වෙන්න interface එක update කළා
interface Crypto {
  name: string;
  price: string | number; 
}
...
const SellCrypto = () => {
  ...
  useEffect(() => {
    // Data formatting logic to handle string/number prices
  ...
  const totalReturn = formData.amount
    ...
    <div className="max-w-md mx-auto p-4">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4 relative">
          ...
          >
            {formData.name || "Select Crypto to Sell"}
          </div>
          {isDropdownOpen && (
        ...
        <div className="flex justify-between mb-4 text-white font-bold uppercase italic text-xs tracking-widest">
          <p>Total Return: </p>
          <p className="text-primary">${totalReturn}</p>
        </div>
